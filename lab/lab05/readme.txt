...
(a)  At compile-time, can we assign xa to ya (No), and vice versa? (Yes)  When
     is a cast required? (assign xa to ya)
(b)  At run-time, if ya references an array of Y's, can we assign it to xa? (Yes)
     Can we then assign it back from xa to ya? (No)
(c)  If xa references an array of X's (that are not Y's), can we assign it to
     ya? (No)  Can we then assign it back from ya to xa? (Yes) Does it make a
     difference if the array of type X[] references objects that are all of
     class Y?  Why do you think this is the case? *
...
(c)  What if the method declaration in the interface has the same return type,
     but a signature with a different parameter type? (won't compile)
(d)  What if the method declaration in the interface has the same return type,
     ...
     parameters have different names? (Works)
...
(a)  Will Java compile the result? (Yes)  Does it make any difference whether the
...
(b)  Write a main() method in the subclass that accesses the constant using the
     ...
     compile the result? (No) Does it make any difference whether the constant in
     the superclass and the constant in the interface have the same value? (No diff)
...
(a)  Define a variable whose static type is the subclass and which references
     ...
     does Java call the superclass method or the subclass method? (subclass)
(b)  Define a variable whose static type is the superclass and which references
     ...
     the superclass method or the subclass method? (No error at compile-time,
     but has an error at run-time)
